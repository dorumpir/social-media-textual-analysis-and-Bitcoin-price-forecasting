...
class Analyzer():
	def __init__(self, will_train, args):
		...
		if self.config.model_type == 'bert':
			self.model = BertForSentimentClassification.from_pretrained(args.model_name_or_path)
		elif self.config.model_type == 'albert':
			self.model = AlbertForSentimentClassification.from_pretrained(args.model_name_or_path)
		elif self.config.model_type == 'distilbert':
			self.model = DistilBertForSentimentClassification.from_pretrained(args.model_name_or_path)
		else:
	...
	def train(self, train_loader, optimizer, criterion):
		...
		for input_ids, attention_mask, labels in tqdm(iterable=train_loader, desc="Training"):
			# Reset gradient
			optimizer.zero_grad()
			# Put input IDs, attention mask, and labels to device
			input_ids, attention_mask, labels = input_ids.to(self.device), attention_mask.to(self.device), labels.to(self.device)
	...
	def classify_sentiment(self, text):
		...
		with torch.no_grad():
			...
			positive_logit = self.model(input_ids=input_ids, attention_mask=attention_mask)
	...
	def get_sia_prob(self, text):
    		# Don't track gradient.
		with torch.no_grad():
			# Tokens are made up of CLS token, text converted to tokens, and SEP token.
			tokens = ['[CLS]'] + self.tokenizer.tokenize(text)[: 500] + ['[SEP]']
			# Convert tokens to input IDs; convert them to tensor, unsqueeze, put it to device.
			input_ids = torch.tensor(self.tokenizer.convert_tokens_to_ids(tokens)).unsqueeze(0).to(self.device)
			# Create attention mask from input IDs.
			attention_mask = (input_ids != 0).long()
